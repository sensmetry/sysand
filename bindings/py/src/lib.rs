...
mod py_errors {
    ...
    /// that owns it, so the message names the step that is missing rather
    /// than leaving an embedder with a class it has never heard of.
    fn unregistered(message: &str) -> PyErr {
        PyRuntimeError::new_err(format!(
            "{message}\n\nnote: sysand's exception classes have not been \
             ...
             package that owns it (its `_errors` module calls \
             `_register_errors`) before calling into the bindings."
        ))
    }
    ...
    macro_rules! registered_exceptions {
        ($($field:ident => $name:ident),+ $(,)?) => {
            ...
            ///
            /// Called by `_errors.py` when the package is imported. The
            /// classes are *not* looked up by module path, because the
            /// Python tree is vendored under other parent packages: there
            /// `sysand._errors` either does not exist, or — when a
            /// standalone sysand happens to be installed alongside —
            /// resolves to a different copy of the classes, so
            /// `except ProjectError` would silently miss.
            ///
            /// The first registration wins; a later one is ignored, since
            /// each installation carries its own copy of this extension and
            /// therefore its own state.
