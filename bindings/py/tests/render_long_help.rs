...
//! `_render_long_help` is what an embedding CLI answers its own `help` with:
//! clap writes to the process's file descriptors, which a Python-level
//! stream swap cannot capture.
