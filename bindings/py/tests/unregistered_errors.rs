...
//! must get a plain, readable `RuntimeError` naming the missing step, not a
//! panic and not another installation's classes.
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    Python::attach(|py| -> PyResult<()> {
        ...
        assert!(
            message.contains("_register_errors"),
            "the error should name the missing registration step, got: {message}"
        );
