...
use sysand_core::resolve::standard::{StandardResolver, standard_resolver};
...
pub fn command_lock<P: AsRef<Path>, S: AsRef<str>>(
    ...
    let combined_resolver: StandardResolver = standard_resolver(
        cwd,
    ...
    let LockOutcome { lock, .. } =
        match sysand_core::commands::lock::do_lock_local_editable(&path, combined_resolver) {
