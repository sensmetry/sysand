            ...
            match (usage.version_constraint, &mut u.version_constraint) {
                ...
                (Some(vc), vc_current @ None) => {
                    log::warn!(
                        ...
                        {SP:>8} but without a version constraint; version constraint(s)\n\
                ...
                (Some(vc_new), Some(vc_current)) => {
                    ...
                    if &vc_new == vc_current {
                        log::warn!(
                            "ignoring usage `{}` with version constraint(s)\n\
                            {SP:>8} `{}`, since it is already present with identical version constraints",
                            usage.resource,
                            vc_new
                        )
                    } else {
                        log::warn!(
                            ...
                            {SP:>8} constraint(s) `{}`; new version constraint(s)\n\
