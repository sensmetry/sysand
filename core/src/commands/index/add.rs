...
pub enum IndexAddError {
    ...
    #[error("adding version failed; no files changed")]
    IoWithCleanupSuccess {
        #[source]
        source: Box<FsIoError>,
    },
    #[error("adding version failed; cleanup also unsuccessful (error {cleanup_error})")]
    IoWithCleanupFailure {
        #[source]
        source: Box<FsIoError>,
        cleanup_error: Box<FsIoError>,
    },
    ...
    #[error("KPAR `{kpar_path}` is invalid")]
    InvalidProject {
    ...
    #[error(
        "project iri is unclear, either it must be provided, or {INFO_FILE_NAME} needs to specify publisher (in which case the iri would be pkg:sysand/<publisher>/<name>)"
    )]
...
pub fn do_index_add<I: AsRef<str>, P: AsRef<Utf8Path>, R: AsRef<Utf8Path>>(
    ...
    let validated_info = info.validate().map_err(map_invalid_project_err)?;
    let _validated_meta = meta.validate().map_err(map_invalid_project_err)?;

    let version = &info.version;
    let semver = &validated_info.version;

    if !semver.build.is_empty() {
        return Err(IndexAddError::VersionHasBuildMetadata {
    ...
    wrapfs::copy(kpar_path, version_path.join(KPAR_FILE_NAME)).map_err(attempt_cleanup)?;
    wrapfs::write(version_path.join(INFO_FILE_NAME), info_str).map_err(attempt_cleanup)?;
    wrapfs::write(version_path.join(META_FILE_NAME), meta_str).map_err(attempt_cleanup)?;
